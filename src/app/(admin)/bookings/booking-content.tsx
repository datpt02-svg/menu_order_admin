...
function sortBookingsForAttention(bookings: BookingItem[]) {
  return [...bookings].sort((left, right) => {
    const leftPriority = left.depositReviewStatus === "submitted" ? 0 : 1;
    const rightPriority = right.depositReviewStatus === "submitted" ? 0 : 1;
...
export function BookingContent({ initialData, highlightedBookingId }: BookingContentProps) {
  ...
  const orderedBookings = useMemo(() => sortBookingsForAttention(bookings), [bookings]);
  ...
  const highlightedBooking = useMemo(
    () => bookings.find((booking) => booking.id === activeHighlightId),
    [activeHighlightId, bookings],
  );
              ...
              <div className="space-y-4">
                <div className="rounded-[24px] border border-[color:var(--line)] bg-white/75 p-5">
                  ...
                  <div className="mt-4 space-y-2 text-sm text-[var(--muted)]">
                    ...
                    {highlightedBooking?.id === selectedBooking.id ? <div className="text-[#8c6d1f]">Booking này vừa được mở từ notification.</div> : null}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <Textarea
                    name="depositReviewNote"
                    placeholder="Ví dụ: ảnh mờ, sai số tiền, bill không đúng booking..."
                    defaultValue={selectedBooking.depositReviewNote || ""}
                    onChange={clearFieldError("depositReviewNote")}
                    invalid={hasError("depositReviewNote")}
                  />
                  <FieldError>{fieldErrors.depositReviewNote}</FieldError>
                </div>

                <div className="flex justify-end gap-3 pt-2">
                  <Button type="button" variant="outline" onClick={handleBookingModalClose}>Đóng</Button>
                  <Button
                    type="button"
                    variant="danger"
                    disabled={isPending}
                    onClick={() => {
                      startTransition(async () => {
                        ...
                        const noteInput = document.querySelector('textarea[name="depositReviewNote"]') as HTMLTextAreaElement | null;
                        formData.append("depositReviewNote", noteInput?.value || "");
